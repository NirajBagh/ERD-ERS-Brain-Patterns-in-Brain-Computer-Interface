%Left Hand movement using AM-EEG Modulation
%% Coding by Niraj Bagh
%% extraction of all Left hand trials
y1=find(y_train==1); %find all left hand trials assign to 1
for i=1:length(y1)
      C3_trials_left(:,i)=x_train(:,1,y1(i));%% C3 for Left hand trials
      X(:,i)=C3_trials_left(:,i); 
      C4_trials_left(:,i)=x_train(:,3,y1(i));%% C4 for left hand trials
      Y(:,i)=C4_trials_left(:,i); 
end
[m,n]=size(X);
fs=128;
ts=1/fs;
...
xlabel('Time (Sec)')
ylabel('Amp(Volt)')
title('Raw EEG signals from a single trial during LH imagination')
legend('C3','C4')
...
ERD_alpha_C3_filtered_avg_LH = (C3_filt_alpha_mag_avg- base_avg_C3)/base_avg_C3 * 100;
...
ERD_alpha_C3_HT_LH= (hilb_alpha_avg_C3 -base_avg_HT_C3)/base_avg_HT_C3 * 100;
...
ERD_alpha_C4_filtered_avg_LH = (C4_mag_alpha_avg- base_avg_C4)/base_avg_C4 * 100; 
...
ERD_alpha_C4_HT_LH= (hilb_alpha_C4_avg-base_avg_HT_C4)/base_avg_HT_C4 * 100;


%% comparision of ERD/ERS patterns between C3 and C4 channels during LH imagination
figure;
subplot(2,1,1);
plot(t1,ERD_alpha_C3_filtered_avg_LH,'r');
hold on;
plot(t1,ERD_alpha_C4_filtered_avg_LH,'b');
xlabel('Time')
ylabel('%ERD/ERS')
title('(a) ERD/ERS patterns using Band Power method during LH imaginationst')
legend('C3','C4')
subplot(2,1,2);
plot(t1,ERD_alpha_C3_HT_LH,'r');
hold on;
plot(t1,ERD_alpha_C4_HT_LH,'b');
xlabel('Time')
ylabel('%ERD/ERS')
title('(b) ERD/ERS patterns using Hilbert transform method during LH imaginations')
legend('C3','C4')
